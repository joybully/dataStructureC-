...
TreeNode* PtrToSuccessor(TreeNode*& tree, ItemType& data);
...
TreeNode* PtrToSuccessor(TreeNode*& tree, ItemType& data) {
    if (tree->left != NULL) PtrToSuccessor(tree->left, data);
    else {
...
bool Imp_IsBST(TreeNode* tree, ItemType& min, ItemType& max) {
    ...
    max = (tree->right == NULL) ? tree->info : tree->right->info;
    return true;
}
...
int main() {
    ...
    item1 = 2;
    tree1.InsertItem(item1);
    outfile.open("tree1.txt");
    tree1.Print(outfile);
    outfile.close();
    std::cout << tree1.LeafCount()<<'\n';
