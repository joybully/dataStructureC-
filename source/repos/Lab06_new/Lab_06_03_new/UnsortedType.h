...
void UnsortedType<ItemType>::DeleteItem_A(ItemType item) {
    ...
    bool found = false;
    // Locate node to be deleted.
    if (location != NULL && found == false && item == listData->info) {
        while (found == false && item == listData->info)
        {
            NodeType<ItemType>* tempLocation;
            tempLocation = location;
            listData = listData->next;		// Delete first node.
...
void UnsortedType<ItemType>::MergeLists(UnsortedType<ItemType>& other, UnsortedType<ItemType>& result) {
    NodeType<ItemType>* location  = (*this).listData;
    ItemType item;
    while (location != NULL) {
        item = location->info;
